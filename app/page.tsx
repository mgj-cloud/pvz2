...
type ToolId = PlantId | "tile" | "guard" | "erase";
type DoomSize = "small" | "medium" | "large";
type Board = Record<string, { type: PlantId; doomSize?: DoomSize; lotusTier?: 1 | 3 }>;
type Feed = { id: number; at: number; cell: string };
type Lane = "操作" | "瓷砖" | "守卫" | "巨人" | "中小怪" | "系统";
...
const initialBoard: Board = {
  ...
  "4-5": { type: "lotus", lotusTier: 3 }, "4-6": { type: "doom", doomSize: "small" }, "5-6": { type: "mandrake" },
};
const initialTiles = new Set(["3-2", "1-6", "2-6", "2-7", "4-6", "5-6"]);
const initialGuards = new Set(["2-7"]);
const lanes: Lane[] = ["操作", "瓷砖", "守卫", "巨人", "中小怪", "系统"];
const HEART_BOOST_AT = 3000;
...
function hasHeartBoost(board: Board, target: string) {
  return Object.entries(board).some(([cell, plant]) => {
    if (plant.type !== "heart") return false;
    const [r, c] = parseKey(cell);
    return target === keyOf(r, c + 1);
  });
}
function boostAt(board: Board, tiles: Set<string>, target: string, at = HEART_BOOST_AT) {
  const targetPlant = board[target];
  if (!targetPlant || (targetPlant.type !== "doom" && targetPlant.type !== "snake")) return 0;
  if (targetPlant.type === "doom" && tiles.has(target)) return 0;
  let boost = 0;
  for (const [cell, plant] of Object.entries(board)) {
    if (plant.type === "coin" && near(cell, target)) boost = Math.max(boost, .2);
    if (plant.type === "lotus" && near(cell, target)) boost = Math.max(boost, plant.lotusTier === 1 ? .5 : .9);
    if (plant.type === "heart" && at >= HEART_BOOST_AT) {
      const [r, c] = parseKey(cell);
...
function acceleratedWindup(base: number, board: Board, tiles: Set<string>, target: string, at: number) {
  const initialBoost = boostAt(board, tiles, target, at);
  const ignoresBoost = board[target]?.type === "doom" && tiles.has(target);
  const heartStartsDuringWindup = at < HEART_BOOST_AT && hasHeartBoost(board, target) && !ignoresBoost;
  if (!heartStartsDuringWindup) {
    return { duration: Math.round(base / (1 + initialBoost)), initialBoost, finalBoost: initialBoost, segmented: false };
  }
  const timeUntilHeart = HEART_BOOST_AT - at;
  const workBeforeHeart = timeUntilHeart * (1 + initialBoost);
  if (workBeforeHeart >= base) {
    return { duration: Math.round(base / (1 + initialBoost)), initialBoost, finalBoost: initialBoost, segmented: false };
  }
  const finalBoost = Math.max(initialBoost, 1);
  return {
    duration: Math.round(timeUntilHeart + (base - workBeforeHeart) / (1 + finalBoost)),
    initialBoost,
    finalBoost,
    segmented: true,
  };
...
function plantEvents(plant: Board[string], cell: string, at: number, board: Board, tiles: Set<string>, map: MapId, strategy: Strategy, source: string): Event[] {
  const sourceLane = source === "瓷砖" ? "瓷砖" : source === "守卫" ? "守卫" : "操作";
  if (plant.type === "doom") {
    const growing = strategy === "costume" && plant.doomSize !== "large";
    const timing = acceleratedWindup(growing ? 2633 : 667, board, tiles, cell, at), windup = timing.duration;
    const accel = timing.segmented
      ? `前 ${ms(HEART_BOOST_AT - at)} 取最高加速 +${timing.initialBoost * 100}%，t=3,000 ms 后切换为 +${timing.finalBoost * 100}%`
      : timing.initialBoost ? `取最高加速 +${timing.initialBoost * 100}%` : "无加速";
    const result: Event[] = [
      ...
      { at: at + windup, lane: "巨人", title: "毁灭菇爆炸", detail: `${cell} · 前摇 ${ms(windup)} · ${accel} · 巨人进入辐射搬运态`, confidence: timing.initialBoost === 0 && !timing.segmented || timing.initialBoost === 1 ? "confirmed" : "estimated", tone: "red" },
  ...
  if (plant.type === "snake") {
    const timing = acceleratedWindup(500, board, tiles, cell, at), windup = timing.duration;
    const accel = timing.segmented
      ? `t=3,000 ms 前 +${timing.initialBoost * 100}%，之后 +${timing.finalBoost * 100}%`
      : timing.initialBoost ? `取最高加速 +${timing.initialBoost * 100}%` : "无加速";
    return [
      { at, lane: sourceLane, title: `${cell} 蛇草启动`, detail: `默认五阶装扮 · ${accel}`, confidence: "confirmed", tone: "green" },
      { at: at + windup, lane: "巨人", title: "蛇草石化判定", detail: `${cell} · 前摇 ${ms(windup)} · 检查 9-3`, confidence: timing.initialBoost === 0 && !timing.segmented || timing.initialBoost === 1 ? "confirmed" : "estimated", tone: "green" },
    ];
  }
  if (plant.type === "mandrake") {
    const windup = 200, width = map === "egypt" ? 82 : 92;
    return [
      { at: at + windup, lane: "中小怪", title: `${cell} 曼德拉子弹发射`, detail: `前摇 ${ms(windup)} · 双速 680px/s · 碰撞宽 ${width}px`, confidence: "confirmed", tone: "cyan" },
...
function simulate(board: Board, tiles: Set<string>, guards: Set<string>, feeds: Feed[], map: MapId, strategy: Strategy) {
  const events: Event[] = [
    ...
    ...(Object.values(board).some(plant => plant.type === "heart") ? [{ at: HEART_BOOST_AT, lane: "系统" as Lane, title: "心叶兰加速启动", detail: "此前 3 秒不计算心叶兰加速；跨过此时刻的前摇按剩余进度分段提速", confidence: "confirmed" as const, tone: "green" }] : []),
  ...
  const doomWaves = new Set<number>();
  const triggerOne = (cell: string, at: number, source: string, scheduleGuard = true) => {
    const plant = board[cell];
    if (!plant) return;
    events.push(...plantEvents(plant, cell, at, board, tiles, map, strategy, source));
    if (scheduleGuard && guards.has(cell)) {
      const durations: Partial<Record<PlantId, number>> = { doom: 2667, snake: 1933, mandrake: 2200, ball: 1133, burdock: 500 };
      const duration = durations[plant.type];
      if (duration !== undefined) {
        const secondAt = at + duration;
        events.push({ at: secondAt, lane: "守卫", title: `${cell} 守卫菇触发二次大招`, detail: `第一次大招完整运行 ${duration}ms 后，同格植物重新启动；随后仍走自身前摇`, confidence: plant.type === "burdock" ? "estimated" : "confirmed", tone: "orange" });
        trigger(cell, secondAt, "守卫", false);
      }
    }
  };
  function trigger(cell: string, at: number, source: string, scheduleGuard = true) {
    const plant = board[cell];
    if (!plant) return;
    if (plant.type !== "doom") return triggerOne(cell, at, source, scheduleGuard);
    if (doomWaves.has(at)) return;
    doomWaves.add(at);
    const doomCells = Object.entries(board).filter(([, candidate]) => candidate.type === "doom").map(([doomCell]) => doomCell);
    const lane: Lane = source === "瓷砖" ? "瓷砖" : source === "守卫" ? "守卫" : "操作";
    events.push({ at, lane, title: "毁灭菇全场联动", detail: `${cell} 启动大招，${doomCells.join("、")} 同时启动；各自按装扮、体型与实时加速计算爆炸时刻`, confidence: "confirmed", tone: "violet" });
    doomCells.forEach(doomCell => triggerOne(doomCell, at, source, scheduleGuard));
  }
  tiles.forEach(cell => {
    const plant = board[cell];
  ...
  feeds.forEach(feed => {
    const plant = board[feed.cell];
    if (!plant) return;
    trigger(feed.cell, feed.at, "手动");
  });
...
export default function Home() {
  ...
  const [guardLinks, setGuardLinks] = useState<Set<string>>(initialGuards);
  const [tool, setTool] = useState<ToolId>("doom"), [doomSize, setDoomSize] = useState<DoomSize>("large");
  const [lotusTier, setLotusTier] = useState<1 | 3>(3);
  ...
  const events = useMemo(() => simulate(board, tiles, guardLinks, feeds, map, strategy), [board, tiles, guardLinks, feeds, map, strategy]);
  const shown = filter === "全部" ? events : events.filter(e => e.lane === filter);
  const result = useMemo(() => timing(events), [events]);

  const reset = () => { setBoard(initialBoard); setTiles(new Set(initialTiles)); setGuardLinks(new Set(initialGuards)); setFeeds([{ id: Date.now(), at: 2675, cell: "2-7" }]); setFeedAt(2675); setSelected("2-7"); };
  const handleCell = (cell: string) => {
    setSelected(cell);
    if (tool === "tile") return setTiles(old => { const next = new Set(old); if (next.has(cell)) next.delete(cell); else next.add(cell); return next; });
    if (tool === "guard") return setGuardLinks(old => { const next = new Set(old); if (next.has(cell)) next.delete(cell); else next.add(cell); return next; });
    if (tool === "erase") {
      setBoard(old => { const next = { ...old }; delete next[cell]; return next; });
      setGuardLinks(old => { const next = new Set(old); next.delete(cell); return next; });
      return setTiles(old => { const next = new Set(old); next.delete(cell); return next; });
    }
    setBoard(old => ({ ...old, [cell]: { type: tool, ...(tool === "doom" ? { doomSize } : {}), ...(tool === "lotus" ? { lotusTier } : {}) } }));
  };
  ...
  return <main>
    <header className="topbar">
      ...
      <div className="head-controls">
        ...
        <label><span>毁菇策略</span><NativeSelect value={strategy} onChange={e => setStrategy(e.target.value as Strategy)}><NativeSelectOption value="costume">装扮 · 体型时差</NativeSelectOption><NativeSelectOption value="split">无装扮 · 全场时差连炸</NativeSelectOption></NativeSelect></label>
    ...
    <div className="workspace">
      <section className="board-panel">
        <div className="heading"><div><em>FORMATION</em><h2>阵型与联动层</h2></div><p><b /> 能量瓷砖　<i /> 守卫菇　<span /> 受加速</p></div>
        <div className={`lawn ${map}`}>
          ...
          <div><div className="col-nums">{[1,2,3,4,5,6,7,8,9].map(n => <span key={n}>{n}</span>)}</div><div className="game-grid">
            {Array.from({ length: 45 }, (_, i) => {
              const cell = keyOf(Math.floor(i / 9) + 1, i % 9 + 1), plant = board[cell], boost = plant ? boostAt(board, tiles, cell, HEART_BOOST_AT) : 0;
              return <button key={cell} className={`cell ${tiles.has(cell) ? "tiled" : ""} ${guardLinks.has(cell) ? "guarded" : ""} ${selected === cell ? "selected" : ""} ${dragging === cell ? "dragging" : ""}`} onClick={() => handleCell(cell)} draggable={!!plant} onDragStart={() => setDragging(cell)} onDragOver={e => e.preventDefault()} onDrop={() => movePlant(cell)} onDragEnd={() => setDragging(null)} aria-label={`${cell} ${plant ? plants[plant.type].name : "空格"}`}>
                {tiles.has(cell) && <Zap className="tile-glyph" />}
                {guardLinks.has(cell) && <span className="guard-glyph">Ⅱ</span>}
                {plant && <span className="plant" style={{ "--plant": plants[plant.type].color } as React.CSSProperties}><strong>{plants[plant.type].short}</strong>{plant.type === "doom" && <small>{plant.doomSize === "large" ? "大" : plant.doomSize === "medium" ? "中" : "小"}</small>}{plant.type === "lotus" && <small>{plant.lotusTier ?? 3}阶</small>}</span>}
        ...
        <div className="palette"><p><MousePointer2 />瓷砖与守卫菇都是格子辅助层；拖动植物时辅助层留在原格</p><div className="tools">
          ...
          <button className={tool === "guard" ? "active" : ""} onClick={() => setTool("guard")}><i className="guard-tool">Ⅱ</i>守卫菇</button>
        ...
        {tool === "lotus" && <label className="size-pick lotus-pick">飞莲等阶 <NativeSelect size="sm" value={lotusTier} onChange={e => setLotusTier(Number(e.target.value) as 1 | 3)}><NativeSelectOption value="1">一阶 · +50%</NativeSelectOption><NativeSelectOption value="3">三阶以上 · +90%</NativeSelectOption></NativeSelect></label>}
    ...
    <footer><div><FlaskConical /><span><b>毁灭菇全场联动</b>任意一株由瓷砖、喂豆或守卫菇启动，所有毁灭菇同时开大并按各自条件结算爆炸。</span></div><div><Sparkles /><span><b>加速判定</b>仅作用于毁菇和蛇草；心叶兰从 t=3,000ms 起生效，跨界前摇分段计算。</span></div><div><Info /><span><b>飞莲等阶</b>一阶 3×3 +50%；三阶及以上 3×3 +90%；每株单独保存等阶。</span></div></footer>
  </main>;
}
